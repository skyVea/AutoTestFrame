 ...
 *
 */
public class DefaultTestReporterListener implements TestReporterListener<BaseTestCase> {
	...
	@Override
	public void generateReport(TestResultContext<BaseTestCase> context) {
		init();
		List<TestResult<BaseTestCase>> results = context.getTestResults();
		try {
		...
		for (int i = 0; i < results.size(); i++) {
			TestResult<BaseTestCase> result = results.get(i);
			test = extent.createTest(result.getTestCase().getCode()); // 显示方法名称
			// test.createNode("子案例" + i); // 创建子案例
			test.assignCategory(result.getRegionId()); // Region
